...
class GPXRunningAnalyzer:
    ...
    def calculate_segment_data(self):
        ...
        # 计算每个段的水平距离
        self.df['horizontal_distance'] = np.sqrt(self.df['segment_distance']**2 - self.df['elevation_change']**2)
        # 处理可能的负数
        self.df['horizontal_distance'] = self.df['horizontal_distance'].fillna(0)  # 如果出现NaN，设为0
    ...
    def get_kilometer_data(self):
        ...
        total_distance_km = self.df['cumulative_distance'].iloc[-1] / 1000
        kilometer_data = []
        ...
        for km in range(1, int(total_distance_km) + 1):
            ...
            # 找到最接近目标距离的点
            closest_idx = (self.df['cumulative_distance'] - target_distance).abs().idxmin()
            closest_row = self.df.iloc[closest_idx]
            
            # 计算该公里段的爬升和下降
            start_idx = (self.df['cumulative_distance'] - (target_distance - 1000)).abs().idxmin()
            end_idx = closest_idx
            
            if start_idx < end_idx:
                segment_df = self.df.iloc[start_idx:end_idx+1]
                climb = segment_df[segment_df['elevation_change'] > 0]['elevation_change'].sum()
                descent = abs(segment_df[segment_df['elevation_change'] < 0]['elevation_change'].sum())
            else:
                climb = 0
                descent = 0
            ...
            kilometer_data.append({
                ...
                '海拔(m)': closest_row['elevation_smoothed'],
                '爬升(m)': climb,
                '下降(m)': descent
            })
    ...
    def generate_excel_report(self):
        ...
        with pd.ExcelWriter(excel_filename, engine='xlsxwriter') as writer:
            ...
            elevation_chart.set_y_axis({'name': '海拔 (m)'})
            ...
            # 创建每公里海拔折线图
            km_elevation_chart = workbook.add_chart({'type': 'line'})
            km_elevation_chart.add_series({
                'name': '每公里海拔',
                'categories': ['每公里数据', 1, 1, km_data_range, 1],  # 距离列
                'values': ['每公里数据', 1, 2, km_data_range, 2],      # 海拔列
                'line': {'width': 2, 'color': 'blue'}
            })
            km_elevation_chart.set_title({'name': '每公里海拔变化'})
            km_elevation_chart.set_x_axis({'name': '距离 (km)'})
            km_elevation_chart.set_y_axis({'name': '海拔 (m)'})
            km_elevation_chart.set_size({'width': 600, 'height': 400})
            km_worksheet.insert_chart('F2', km_elevation_chart)
            ...
            climb_descent_chart.add_series({
                ...
                'values': ['每公里数据', 1, 3, km_data_range, 3],      # 爬升列
                'fill': {'color': 'green'}
            })
            climb_descent_chart.add_series({
                'name': '下降',
                'categories': ['每公里数据', 1, 0, km_data_range, 0],  # 公里数列
                'values': ['每公里数据', 1, 4, km_data_range, 4],      # 下降列
                'fill': {'color': 'red'}
            })
            ...
            km_worksheet.insert_chart('F20', climb_descent_chart)
    ...
    def plot_elevation_profile(self):
        ...
        for km_data in self.results.get('kilometer_data', []):
            ...
            ax.annotate(f"{km_data['公里']}km\n{km_data['海拔(m)']:.0f}m", 
                    xy=(x_pos, y_pos),
                    xytext=(10, text_y_offset),
                    textcoords='offset points',
                    fontsize=8,
                    bbox=dict(boxstyle="round,pad=0.3", fc="yellow", alpha=0.7),
                    arrowprops=dict(arrowstyle="->", connectionstyle=f"arc3,rad={arrow_rad}"))
