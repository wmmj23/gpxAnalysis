...
class GPXRunningAnalyzer:
    ...
    # 识别连续大坡度路段
    def find_challenging_sections(self, threshold=None, min_length=None):
    ...
    # 识别段落大坡度路段
    def find_segment_challenging_sections(self, min_length=None, avg_slope_threshold=None, 
                                     steep_slope_threshold=None, steep_ratio_threshold=None):
        ...
        # 如果未提供参数，则使用默认值
        if min_length is None:
            min_length = 500  # 默认段落最小长度
        if avg_slope_threshold is None:
            avg_slope_threshold = 5  # 默认平均坡度阈值
        if steep_slope_threshold is None:
            steep_slope_threshold = 15  # 默认陡坡阈值
        if steep_ratio_threshold is None:
            steep_ratio_threshold = 30  # 默认陡坡比例阈值
    ...
    def analyze(self):
        ...
        try:
            ...
            # 获取用户输入的连续大坡度路段参数
            print("\n请设置连续大坡度路段(连续点的坡度大于坡度阈值)的识别参数（直接回车使用默认值）:")
            try:
                threshold_input = input(f"坡度阈值，海拔变化/水平距离*100%（默认8%）: ")
                threshold = float(threshold_input) if threshold_input else 8
                
                min_length_input = input(f"最小长度，即连续多长距离视为挑战路段（默认200米）: ")
                min_length_continuous = float(min_length_input) if min_length_input else 200
            except ValueError:
                print("输入无效，使用默认参数：坡度阈值8%，最小长度200米")
                threshold = 8
                min_length_continuous = 200
            
            print("识别连续大坡度路段...")
            self.find_challenging_sections(threshold=threshold, min_length=min_length_continuous)  
        
            # 获取用户输入的段落大坡度路段参数
            print("\n请设置段落大坡度路段(段落中一定百分比的点坡度大于坡度阈值)的识别参数（直接回车使用默认值）:")
            try:
                min_length_input = input(f"段落最小长度（默认500米）: ")
                min_length_segment = float(min_length_input) if min_length_input else 500
                
                avg_slope_input = input(f"平均坡度阈值，即段落所有点坡度的平均值要高于多少（默认5%）: ")
                avg_slope_threshold = float(avg_slope_input) if avg_slope_input else 5
                
                steep_slope_input = input(f"陡坡阈值，即定义陡坡的坡度（默认15%）: ")
                steep_slope_threshold = float(steep_slope_input) if steep_slope_input else 15
                
                steep_ratio_input = input(f"陡坡比例阈值，即该段落陡坡占比（默认30%）: ")
                steep_ratio_threshold = float(steep_ratio_input) if steep_ratio_input else 30
            except ValueError:
                print("输入无效，使用默认参数：段落最小长度500米，平均坡度阈值5%，陡坡阈值15%，陡坡比例阈值30%")
                min_length_segment = 500
                avg_slope_threshold = 5
                steep_slope_threshold = 15
                steep_ratio_threshold = 30
            
            print("识别段落大坡度路段...")
            self.find_segment_challenging_sections(
                min_length=min_length_segment,
                avg_slope_threshold=avg_slope_threshold,
                steep_slope_threshold=steep_slope_threshold,
                steep_ratio_threshold=steep_ratio_threshold
            )
